...
public class ApService {
    ...
    public void getData(SearchDto dto) {
        ...
        List<ApartmentInfoEntity> list = null;
        try {
            jsonNode = om.readTree(result);
            list = om.convertValue(jsonNode.path("response").path("body").path("items").path("item"), new TypeReference<List<ApartmentInfoEntity>>(){});
